...
## 发布状态 — v2.5.0 高级同步冲突处理

当前仓库在托管更新器、剪贴板历史、智能增强、模板、局域网同步、WebDAV 云同步、Phase 4 访问安全、原生生物识别集成和数据库静态加密的基础上，新增了 **高级同步冲突处理**。

### v2.5.0 亮点

- **智能冲突检测**：当同一条目在多个设备上被修改时自动识别冲突
- **可配置解决策略**：支持最后写入优先 / 本地优先 / 远程优先 / 手动解决四种策略
- **手动解决 UI**：并排对比本地与远程版本，一键选择保留哪个
- **冲突日志**：完整的冲突解决历史记录，支持逐条管理和批量清理
