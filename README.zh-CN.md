...
## 发布状态 — v2.9.0 智能搜索与知识组织

当前主分支已经形成覆盖剪贴板历史、智能增强、模板、安全保护、局域网 / WebDAV 同步、托管更新和插件扩展的完整桌面应用。v2.9.0 进一步强化了 **智能搜索与知识组织**。

### v2.9.0 亮点

- **智能分组**：基于 N-gram / TF-IDF 相似度自动聚类相近条目，并生成更易理解的分组标签
- **标签建议**：从相似条目与已有标签分布中推荐可复用标签，直接展示在卡片内
- **相关条目推荐**：展示最相近的 5 条历史内容及相似度百分比，帮助快速联想与复用
- **搜索结果重排序**：使用 TF-IDF 余弦相似度对 FTS5 召回结果重排，改善模糊查询排序效果
- **相似度评分扩展点**：`SimilarityScorer` trait 为未来接入向量 / 嵌入式后端保留扩展边界
...
- **剪贴板历史** -- 自动捕获复制内容并去重
- **全文搜索** -- 基于 SQLite FTS5 的快速搜索，支持中文拼音全拼与首字母匹配
- **分类筛选** -- 按内容类型浏览历史记录
- **全局快捷键** -- `Cmd/Ctrl + Shift + V` 切换剪贴板面板
- **快捷粘贴** -- `Cmd/Ctrl + Shift + 1` 呼出轻量弹层，按数字键即可快速粘贴
- **批量操作** -- 多选模式下支持批量删除、批量收藏/取消收藏、批量打标签、合并复制，以及全选/反选/清空选择
- **虚拟滚动** -- 大量剪贴板记录场景下仅渲染可视区域，保障列表滚动流畅
...
- **智能搜索与知识组织** -- 通过智能分组、标签建议、相关条目和搜索重排序，让历史内容更容易整理、检索与再次复用

### 安全、同步与扩展

- **访问安全** -- 密码锁、手动锁定（设置面板 + 托盘菜单）、超时自动锁定、托盘/热键唤起拦截与命令级访问控制
- **数据库加密** -- 可选的 AES-256-GCM 剪贴板数据静态加密，密钥存储在系统密钥链中
- **局域网同步** -- 基于 mDNS + WebSocket 的加密点对点同步
- **WebDAV 云同步** -- 带设备注册表、轮询和限流的端到端加密云同步
- **同步冲突处理** -- 智能冲突检测，可配置解决策略与手动对比 UI
- **托管更新器** -- 后台检查更新、镜像端点、带进度的安装包下载、签名校验和安装切换
- **插件扩展系统** -- 基于本地 manifest 的插件发现、校验、启停管理、受信内容处理扩展钩子，以及 `SimilarityScorer` 相似度评分扩展点
...
- [x] **核心剪贴板工作流**：后台监听、去重历史、内容分类、搜索界面、全局快捷键、系统托盘与设置
- [x] **智能增强能力**：敏感检测、内容转换、标签、图片剪贴板、使用统计与可复用模板
- [x] **多设备同步**：局域网同步、设备配对、加密 WebSocket 传输、WebDAV 云同步与冲突处理
- [x] **访问安全体系**：应用锁、安全密码存储、启动锁定门禁、托盘/热键拦截、自动锁定、命令级守卫、原生生物识别与数据库静态加密
- [x] **智能搜索与知识组织**：智能分组、标签建议、相关条目推荐、搜索重排序、批量操作、虚拟滚动与快捷粘贴弹层
- [x] **插件扩展系统**：本地插件发现、校验、启用/禁用管理、受信转换钩子，以及面向未来排序后端的 `SimilarityScorer` 相似度评分扩展点

### 后续方向

- [ ] **更强的平台级安全增强**：例如更准确的空闲检测、更自然的原生解锁体验，以及更多受保护入口的一致性治理
- [ ] **更丰富的插件能力**：从当前转换动作扩展到更广义的内容处理与相似度后端接入
- [ ] **进一步增强搜索与知识组织**：继续提升分组质量、推荐可解释性与结果相关性，同时保持轻量和本地优先
