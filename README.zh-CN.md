...
当前仓库在拼音模糊搜索、高级同步冲突处理、托管更新器、剪贴板历史、智能增强、模板、局域网同步、WebDAV 云同步、Phase 4 访问安全、原生生物识别集成和数据库静态加密的基础上，新增了 **插件 / 扩展系统**。

### v2.7.0 亮点

- **插件 / 扩展系统**：支持从本地 `plugins/` 目录发现插件，进行 manifest 校验、受信 builtin handler 绑定和持久化启停管理
- **插件管理 UI**：可在设置面板查看有效 / 无效插件、检查元数据，并直接启用或禁用插件
- **插件转换接入**：插件提供的转换动作会直接出现在 Transform 菜单中，并通过独立的后端插件命令链路执行
- **内置示例插件**：随仓库附带 `markdown-tools` 示例插件，用于验证 manifest 加载、registry 调度与端到端转换行为
