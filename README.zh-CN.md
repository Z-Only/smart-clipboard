...
当前仓库已经包含 **Phase 3 局域网同步** 的端到端加密实现。本次版本提供：

- 桌面端新增 Sync 同步管理面板
- 支持编辑设备名称和同步端口
- 支持基于真实 mDNS / DNS-SD 的设备发现列表，并自动刷新与去重
- 支持设备配对 / 取消配对流程，配对时进行 X25519 密钥交换
- 支持按设备开启或关闭同步
- 支持持久化保存同步配置和已配对设备信息（含加密密钥材料）
- **端到端加密**：X25519 Diffie-Hellman 密钥交换、HKDF-SHA256 共享密钥派生、AES-256-GCM 加密消息传输
- **密钥指纹验证**：生成人类可读的密钥指纹，用于配对确认
- **WebSocket 传输层**：自动连接、ping/pong 心跳、断线重连退避、实时连接状态

### 当前范围

这是一次 **Phase 3 交付**，覆盖局域网发现、传输层和端到端加密。以下能力仍将在后续版本继续实现：
...
- X25519 密钥对生成，密钥持久化存储在应用配置中
- 设备配对时的 HKDF-SHA256 共享密钥派生
- 所有同步协议消息的 AES-256-GCM 加密/解密
- 人类可读的密钥指纹生成，用于配对验证
- `KeyVerification` 协议消息类型，支持双端确认流程
- 数据库迁移：为已配对设备表添加 `fingerprint` 列
- 加密模块 7 个单元测试（密钥派生、加密往返、指纹格式、错误处理）
- 基于 `_smartclip._tcp.local.` 的真实 mDNS / DNS-SD 服务广播
- Phase 3 WebSocket 传输层：自动连接、心跳、断线重连退避
