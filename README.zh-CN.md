...
当前仓库在快捷粘贴面板、插件扩展系统、拼音模糊搜索、高级同步冲突处理、托管更新器、剪贴板历史、智能增强、模板、局域网同步、WebDAV 云同步、Phase 4 访问安全、原生生物识别集成和数据库静态加密的基础上，新增了 **智能搜索与知识组织**。

### v2.9.0 亮点

- **智能分组**：基于 N-gram/TF-IDF 相似度自动将条目聚类，自动生成分组标签，可通过新增的侧边面板访问
- **标签建议**：基于内容相似度为条目推荐标签，以内联芯片形式展示，支持一键接受或忽略
- **相关条目推荐**：可折叠区域展示相似度最高的 5 条相关条目及相似度百分比
- **搜索结果重排序**：使用 TF-IDF 余弦相似度对 FTS5 结果进行重排序，显著改善模糊查询的排序效果
- **Phase B 扩展点**：`SimilarityScorer` trait 支持未来通过插件系统接入向量/嵌入后端
...
- **智能分组** -- 基于 N-gram/TF-IDF 相似度自动将条目聚类，自动生成分组标签，提供专属侧边面板
- **标签建议** -- 基于内容相似度为条目推荐标签，内联展示在条目卡片上
- **相关条目推荐** -- 可折叠区域展示相似度最高的相关条目及相似度百分比
