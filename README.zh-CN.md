...
当前仓库已经包含 **托管更新器** 实现，并叠加了剪贴板历史、智能增强、模板、局域网同步、WebDAV 云同步和 Phase 4 访问安全能力。

### v2.2.0 亮点

- **自动检查更新**：支持可配置的后台定时检查（6小时 / 12小时 / 每天 / 每周）
- **手动检查**：在设置中点击版本行可立即检查更新
- **镜像端点**：支持自定义 HTTPS 镜像，使用 `{url}` 占位符进行 CDN 加速
- **带进度的安装包下载**：下载更新安装包时显示实时进度
- **签名校验**：使用 Minisign 对下载的安装包进行签名验证
- **待安装更新管理**：已下载的更新在重启后仍然保留，随时可安装或丢弃
- **安装切换**：启动下载的安装包，提供退出按钮完成更新
- **阶段感知 UI**：设置面板根据更新阶段展示对应的状态、控件和操作
- **仅 Wi-Fi 下载**：可选仅在 Wi-Fi 网络下自动下载更新
