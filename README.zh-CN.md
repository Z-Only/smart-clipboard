...
当前仓库在插件扩展系统、拼音模糊搜索、高级同步冲突处理、托管更新器、剪贴板历史、智能增强、模板、局域网同步、WebDAV 云同步、Phase 4 访问安全、原生生物识别集成和数据库静态加密的基础上，新增了 **快捷粘贴面板**。

### v2.8.0 亮点

- **快捷粘贴面板**：全局快捷键（`Cmd/Ctrl+Shift+1`）呼出轻量弹层，展示最近 9 条剪贴板记录，按数字键 1-9 即可快速粘贴，支持方向键导航、Esc 关闭和输入即搜索切换
- **可配置快捷键**：可在设置中自定义快捷粘贴的触发快捷键和显示条目数
- **无缝搜索过渡**：在快捷粘贴弹层中输入任意字符，自动切换到完整搜索界面并预填已输入内容
...
- **拼音模糊搜索** -- 支持按汉字、全拼、首字母检索中文条目，并带短查询兜底
...
- **插件 / 扩展系统** -- 基于本地 manifest 的插件发现、校验、启停管理与受信内容处理扩展钩子
...
- [x] **高级同步冲突处理**：提供更智能的合并 / 冲突解决策略
- [x] **拼音模糊搜索**：支持中文条目的汉字 / 全拼 / 首字母检索
- [x] **插件 / 扩展系统**：支持从本地目录发现扩展、管理启停状态，并将插件动作接入转换链路
- [x] **快速粘贴面板**：轻量级键盘驱动的剪贴板快速粘贴面板，支持基于数字键 1-9 的快速粘贴，无缝搜索切换
