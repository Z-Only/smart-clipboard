...
当前仓库在高级同步冲突处理、托管更新器、剪贴板历史、智能增强、模板、局域网同步、WebDAV 云同步、Phase 4 访问安全、原生生物识别集成和数据库静态加密的基础上，新增了 **拼音模糊搜索**。

### v2.6.0 亮点

- **拼音模糊搜索**：支持按中文原文、全拼、首字母快速检索中文剪贴板条目
- **短输入兜底**：`zn`、`z` 这类极短输入会自动走 LIKE 兜底逻辑
- **大小写无关首字母**：如 `ZNJTB` 可直接命中对应的拼音首字母索引
- **搜索提示更清晰**：搜索框占位文案明确提示支持“内容 / 拼音 / 首字母”
