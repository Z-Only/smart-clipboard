...
impl ClusterEngine {
    pub fn new(scorer: Arc<dyn SimilarityScorer>, similarity_threshold: f64, max_cluster_size: usize) -> Self {
        Self {
            scorer,
    ...
    pub fn assign_entry(&self, entry_content: &str, existing_clusters: &[ClusterSummary]) -> Option<i64> {
        ...
        for cluster in existing_clusters {
            let score = self.scorer.score(entry_content, &cluster.representative_content);
            if score > best_score {
                best_score = score;
    ...
    pub fn recluster(&self, entries: &[(i64, String)]) -> Vec<ClusterResult> {
        ...
        let entries_to_cluster = if entries.len() > 1000 {
            ...
            sampled.iter().map(|&(id, ref content)| (*id, content.clone())).collect::<Vec<_>>()
        } else {
            entries.to_vec()
        };
