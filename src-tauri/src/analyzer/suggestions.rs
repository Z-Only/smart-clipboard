...
impl TagSuggester {
    pub fn new(scorer: Arc<dyn SimilarityScorer>, min_confidence: f64, max_suggestions: usize) -> Self {
        Self {
            scorer,
            min_confidence,
            max_suggestions,
    ...
    pub fn suggest_tags(
        ...
        let mut tag_scores: std::collections::HashMap<String, (f64, usize)> = std::collections::HashMap::new();
        let mut tag_info_map: std::collections::HashMap<String, TagInfo> = std::collections::HashMap::new();
        ...
        let mut results: Vec<TagSuggestionResult> = tag_scores
            ...
            .filter_map(|(tag_name, (total_score, count))| {
                ...
                if confidence >= self.min_confidence {
                    tag_info_map.get(&tag_name).map(|tag_info| {
                        TagSuggestionResult {
                            tag: tag_info.clone(),
                            confidence,
                        }
    ...
    fn test_suggest_tags_with_matching_content() {
        ...
        let tagged_entries = vec![
            (
                ...
                vec![
                    TagInfo { id: 1, name: "javascript".to_string() },
                    TagInfo { id: 2, name: "async".to_string() },
                ],
            ),
            (
                "async function typescript".to_string(),
                vec![
                    TagInfo { id: 3, name: "typescript".to_string() },
                    TagInfo { id: 2, name: "async".to_string() },
                ],
            ),
        ];
    ...
    fn test_suggest_tags_below_threshold_returns_empty() {
        ...
        let tagged_entries = vec![
            (
                ...
                vec![
                    TagInfo { id: 1, name: "unrelated".to_string() },
                ],
            ),
        ];
    ...
    fn test_suggest_tags_respects_max_suggestions() {
        ...
        let tagged_entries = vec![
            (
                ...
                vec![
                    TagInfo { id: 1, name: "tag1".to_string() },
                    TagInfo { id: 2, name: "tag2".to_string() },
                    TagInfo { id: 3, name: "tag3".to_string() },
                ],
            ),
            (
                "hello world function".to_string(),
                vec![
                    TagInfo { id: 4, name: "tag4".to_string() },
                    TagInfo { id: 5, name: "tag5".to_string() },
                ],
            ),
        ];
