...
pub async fn trigger_recluster(
    ...
    let engine = crate::analyzer::ClusterEngine::new(
        scorer,
        app_config.cluster_similarity_threshold,
        50,
    );
    ...
    let entries_raw = db.get_unclustered_entries(5000).map_err(|e| e.to_string())?;
    ...
    for result in &cluster_results {
        let cluster_id = db.upsert_cluster(&result.label).map_err(|e| e.to_string())?;
...
pub async fn accept_tag_suggestion(
    ...
    require_unlocked(&lock)?;
    db.add_tag_to_entry(entry_id, tag_id).map_err(|e| e.to_string())?;
    // Remove this specific suggestion
    db.dismiss_tag_suggestions(entry_id).map_err(|e| e.to_string())
}
