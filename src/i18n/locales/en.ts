export default {
  app: {
    name: 'Smart Clipboard',
    entries: '{count} entries',
    settings: 'Settings',
    statistics: 'Statistics',
  },
  search: {
    placeholder: 'Search text / pinyin / initials (e.g. znjtb)',
  },
  categories: {
    all: 'All',
    favorites: 'Favorites',
    url: 'Links',
    email: 'Email',
    code: 'Code',
    json: 'JSON',
    filepath: 'Files',
    color: 'Colors',
    phone: 'Phone',
    address: 'Address',
    image: 'Images',
    tags: 'Tags',
    templates: 'Templates',
    text: 'Text',
  },
  entry: {
    categoryLabels: {
      url: 'URL',
      email: 'Email',
      color: 'Color',
      filepath: 'File',
      json: 'JSON',
      xml: 'XML',
      code: 'Code',
      phone: 'Phone',
      address: 'Addr',
      image: 'Image',
      tags: 'Tags',
      text: 'Text',
    },
    sensitive: 'Sensitive',
    expiresIn: 'Expires in {n}m',
    toggleFavorite: 'Toggle favorite',
    delete: 'Delete',
    justNow: 'just now',
    minutesAgo: '{n}m ago',
    hoursAgo: '{n}h ago',
    daysAgo: '{n}d ago',
  },
  list: {
    loading: 'Loading...',
    loadingMore: 'Loading more...',
    noEntries: 'No entries yet',
    noEntriesHint: 'Copy something to get started',
    today: 'Today',
    yesterday: 'Yesterday',
    multiSelect: 'Multi-select',
    exitMultiSelect: 'Done',
    toggleSelection: 'Toggle selection',
    selectedCount: '{count} selected',
    batchDelete: 'Delete selected',
    batchFavorite: 'Favorite selected',
    batchUnfavorite: 'Unfavorite selected',
    batchCopy: 'Merge copy',
    batchTag: 'Tag selected',
    virtualizedHint: 'Virtualized list enabled',
    selectAllLoaded: 'Select all',
    invertSelection: 'Invert',
    clearSelection: 'Clear',
  },
  settings: {
    title: 'Settings',
    maxEntries: 'Maximum entries',
    maxEntriesHint: 'Oldest non-favorite entries are removed beyond this limit',
    retentionDays: 'Retention days',
    retentionDaysHint: 'Entries older than this are automatically removed',
    monitorInterval: 'Monitor interval (ms)',
    monitorIntervalHint: 'How often to check for clipboard changes (restart required)',
    sensitiveExpiry: 'Sensitive content expiry (min)',
    sensitiveExpiryHint:
      'Auto-delete sensitive content (passwords, keys) after this time. 0 = no auto-expiry',
    excludedApps: 'Excluded apps',
    excludedAppsPlaceholder: 'One app name per line',
    excludedAppsHint: 'Clipboard content from these apps will be ignored',
    autoStart: 'Start on login',
    autoStartHint: 'Launch automatically when you log in',
    resetDefaults: 'Reset defaults',
    save: 'Save',
    language: 'Language',
    languageHint: 'Select display language',
    appearance: 'Appearance',
    appearanceHint: 'Switch between light and dark mode',
    appearanceSystem: 'System',
    appearanceLight: 'Light',
    appearanceDark: 'Dark',
    themeColor: 'Theme color',
    themeColorHint: 'Choose accent color for the interface',
    themeZinc: 'Zinc',
    themeBlue: 'Blue',
    themeGreen: 'Green',
    themeRose: 'Rose',
    themeOrange: 'Orange',
    themeViolet: 'Violet',
    plugins: {
      title: 'Plugins',
      hint: 'Manage installed plugins and their availability',
      empty: 'No plugins found',
      invalid: 'Invalid plugin',
      id: 'ID: {id}',
      kind: 'Kind: {kind}',
      version: 'Version: {version}',
    },
    updater: {
      title: 'Application Updates',
      hint: 'Configure background checks and mirror endpoints',
      autoCheck: 'Automatically check for updates',
      checkFrequency: 'Check frequency',
      every6Hours: 'Every 6 hours',
      every12Hours: 'Every 12 hours',
      daily: 'Daily',
      weekly: 'Weekly',
      autoDownload: 'Automatically download latest installer',
      wifiOnly: 'Only auto-download on Wi-Fi',
      mirrors: 'Mirror endpoints',
      mirrorsPlaceholder: 'One https://.../{url} entry per line',
      invalidMirror: 'Mirror must start with https:// and include {url}',
      currentVersion: 'Current version {version}',
      clickToCheck: 'Click to check for updates',
      readyTitle: 'Installer ready for version {version}',
      availableTitle: 'Version {version} is available',
      downloadInstaller: 'Download installer',
      installingTitle: 'Installer handoff started',
      installingHint: 'You may need to quit the app to finish installation',
      quitNow: 'Quit app now',
      installAndRestart: 'Install and restart',
      discardPending: 'Cancel and delete installer',
      phase: {
        idle: 'Idle',
        checking: 'Checking for updates...',
        update_available: 'Update available',
        downloading: 'Downloading installer...',
        ready_to_install: 'Installer ready to install',
        up_to_date: 'You are up to date',
        installing: 'Installing update...',
        error: 'Update check failed',
      },
    },
  },
  transforms: {
    title: 'Transform',
    uppercase: 'UPPERCASE',
    lowercase: 'lowercase',
    titleCase: 'Title Case',
    urlEncode: 'URL Encode',
    urlDecode: 'URL Decode',
    jsonFormat: 'Format JSON',
    jsonCompact: 'Compact JSON',
    base64Encode: 'Base64 Encode',
    base64Decode: 'Base64 Decode',
    trim: 'Trim Whitespace',
    htmlEscape: 'HTML Escape',
    htmlUnescape: 'HTML Unescape',
    copied: 'Transformed & copied!',
  },
  tags: {
    addTag: 'Add tag',
    createTag: 'Create tag...',
    noTags: 'No tags yet',
    deleteTag: 'Delete tag',
    filterByTag: 'Filter by tag',
    batchTitle: 'Batch tags',
    batchHint: 'Apply tags to {count} selected items',
    applyBatch: 'Apply tags',
    createAction: 'Create',
    batchModeAppend: 'Append tags',
    batchModeReplace: 'Replace tags',
  },
  templates: {
    title: 'Templates',
    create: 'New Template',
    edit: 'Edit Template',
    delete: 'Delete',
    name: 'Name',
    namePlaceholder: 'Template name...',
    content: 'Content',
    contentPlaceholder: 'Hello {{name}}, welcome to {{company}}!',
    category: 'Category',
    placeholder: 'Placeholder',
    placeholderHint: 'Use {{name}} syntax',
    detectedPlaceholders: 'Detected Placeholders',
    use: 'Use Template',
    fillPlaceholders: 'Fill Placeholders',
    preview: 'Preview',
    noTemplates: 'No templates yet. Create one to get started!',
    confirmDelete: 'Delete this template?',
    cancel: 'Cancel',
    save: 'Save',
    copyToClipboard: 'Copy to Clipboard',
    copied: 'Copied!',
  },
  sync: {
    title: 'Sync',
    subtitle: 'Manage LAN sync devices and connection settings',
    button: 'Sync',
    tab: {
      lan: 'LAN Sync',
      webdav: 'Cloud Sync',
    },
    panel: {
      enabled: 'Sync enabled',
      deviceName: 'Device name',
      port: 'Port',
      pairedDevices: 'Paired devices',
      discoveredDevices: 'Discovered devices',
      activeDevices: 'Active devices',
      status: 'Status',
    },
    actions: {
      refresh: 'Refresh',
      refreshing: 'Refreshing...',
      save: 'Save',
      saving: 'Saving...',
      close: 'Close',
      pair: 'Pair',
      unpair: 'Unpair',
    },
    loading: 'Loading sync data...',
    statusValues: {
      idle: 'Idle',
      discovering: 'Discovering',
      connecting: 'Connecting',
      connected: 'Connected',
      online: 'Online',
      offline: 'Offline',
      disabled: 'Disabled',
      pairing: 'Pairing',
      error: 'Error',
      unknown: 'Unknown',
    },
    hints: {
      enabled: 'Allow this device to discover and sync with nearby devices on your network.',
    },
    placeholders: {
      deviceName: 'My MacBook',
    },
    empty: {
      paired: 'No paired devices yet.',
      discovered: 'No devices discovered on the current network.',
    },
    device: {
      syncOn: 'Sync enabled for this device',
      syncOff: 'Sync disabled for this device',
      availableToPair: 'Available to pair',
    },
    pairDialog: {
      title: 'Pair with device?',
      message: 'You are about to pair with the following device:',
      fingerprint: 'Device fingerprint',
      confirm: 'Confirm Pair',
      cancel: 'Cancel',
      warning: 'Make sure you trust this device before pairing.',
    },
  },
  webdav: {
    title: 'Cloud Sync (WebDAV)',
    description: 'Sync clipboard across networks via WebDAV server',
    serverUrl: 'Server URL',
    serverUrlPlaceholder: 'https://dav.example.com/dav',
    username: 'Username',
    password: 'Password',
    syncPassword: 'Sync Encryption Password',
    syncPasswordHint:
      'Used to encrypt all data on the server. All devices must use the same password.',
    connect: 'Connect',
    disconnect: 'Disconnect',
    connecting: 'Connecting...',
    connected: 'Connected',
    disconnected: 'Disconnected',
    testConnection: 'Test Connection',
    testSuccess: 'Connection successful!',
    testFailed: 'Connection failed',
    status: 'Status',
    lastSync: 'Last Sync',
    cloudEntries: 'Cloud Entries',
    registeredDevices: 'Registered Devices',
    rateLimit: 'API Rate Limit',
    rateLimitInfo: '{available} / {capacity} requests available',
    pollInterval: 'Poll Interval',
    pollIntervalUnit: 'seconds',
    syncImages: 'Sync Images',
    syncSensitive: 'Sync Sensitive Items',
    remotePath: 'Remote Path',
    maxCloudEntries: 'Max Cloud Entries',
    removeDevice: 'Remove',
    removeDeviceConfirm: 'Remove device "{name}" from cloud sync?',
    noDevices: 'No devices registered',
    error: 'Error',
    settings: 'Settings',
    advanced: 'Advanced Settings',
    rateLimitCapacity: 'Rate Limit (requests)',
    rateLimitRefillMinutes: 'Rate Limit Refill (minutes)',
    triggerSync: 'Sync Now',
    syncTriggered: 'Sync triggered, {count} new entries downloaded',
  },
  statistics: {
    title: 'Statistics',
    totalEntries: 'Total Entries',
    totalFavorites: 'Favorites',
    storageSize: 'Storage Size',
    categoryBreakdown: 'By Category',
    dailyActivity: 'Daily Activity (30 days)',
    mostUsed: 'Most Used',
    noData: 'No data yet',
    times: '{n} times',
  },
  lock: {
    title: 'App locked',
    settingsTitle: 'App Lock',
    settingsHint: 'Protect clipboard access with local authentication',
    autoLock: 'Auto lock (seconds)',
    autoLockHint: 'Require unlock again after this idle time. 0 = disabled',
    biometric: 'Biometric unlock',
    biometricHint: 'Use system biometrics when available, otherwise fall back to password',
    setPassword: 'Set password',
    lockNow: 'Lock now',
    passwordPlaceholder: 'Enter app lock password',
    currentPasswordPlaceholder: 'Current password (blank for first setup)',
    newPasswordPlaceholder: 'New password',
    unlockWithPassword: 'Unlock with password',
    unlockWithBiometric: 'Unlock with biometrics',
    failedAttempts: 'Failed attempts: {count}',
    reasonStartup: 'Unlock required after app launch',
    reasonAutoLock: 'The app was auto-locked due to inactivity',
    reasonManual: 'The app was locked manually',
    reasonProtected: 'Unlock required before showing protected content',
    reasonDefault: 'Enter your password to continue',
  },
  conflict: {
    title: 'Sync Conflicts',
    subtitle: 'Review and resolve conflicts between local and remote changes',
    tab: 'Conflicts',
    pendingCount: '{count} pending',
    noPending: 'No pending conflicts',
    noPendingHint: 'All sync conflicts have been resolved',
    strategy: {
      title: 'Resolution Strategy',
      hint: 'Choose how conflicts are automatically resolved',
      'last-write-wins': 'Last write wins',
      'last-write-wins-hint': 'The most recently modified version is kept',
      'local-first': 'Local first',
      'local-first-hint': 'Always keep the local version',
      'remote-first': 'Remote first',
      'remote-first-hint': 'Always keep the remote version',
      manual: 'Manual',
      'manual-hint': 'Always ask before resolving',
    },
    keepLog: 'Keep conflict log',
    keepLogHint: 'Record all resolved conflicts for review',
    maxLogEntries: 'Max log entries',
    resolve: {
      title: 'Resolve Conflict',
      description: 'The same entry was modified on multiple devices. Choose which version to keep.',
      localVersion: 'Local Version',
      remoteVersion: 'Remote Version',
      fromDevice: 'From {device}',
      modifiedAt: 'Modified {time}',
      contentType: 'Type: {type}',
      keepLocal: 'Keep Local',
      keepRemote: 'Keep Remote',
      dismiss: 'Skip',
      nextConflict: 'Next Conflict',
      remaining: '{count} conflicts remaining',
    },
    log: {
      title: 'Conflict Log',
      empty: 'No conflict history yet',
      emptyHint: 'Resolved conflicts will appear here',
      clearAll: 'Clear log',
      clearConfirm: 'Clear all conflict history?',
      entry: {
        localContent: 'Local',
        remoteContent: 'Remote',
        device: 'Device',
        strategy: 'Strategy',
        outcome: 'Outcome',
        resolvedAt: 'Resolved',
      },
      outcome: {
        'kept-local': 'Kept local',
        'kept-remote': 'Kept remote',
        'auto-resolved': 'Auto-resolved',
        dismissed: 'Dismissed',
      },
    },
  },
  encryption: {
    settingsTitle: 'Database Encryption',
    settingsHint: 'Encrypt clipboard data stored on disk using AES-256-GCM',
    enabled: 'Encryption enabled',
    enable: 'Enable encryption',
    disable: 'Disable encryption',
    migrating: 'Migrating data...',
    statusEncrypted: '{count} entries encrypted',
    statusPlaintext: '{count} entries unencrypted',
    enableConfirm: 'This will encrypt all existing clipboard entries. Continue?',
    disableConfirm: 'This will decrypt all entries and remove the encryption key. Continue?',
  },
};
