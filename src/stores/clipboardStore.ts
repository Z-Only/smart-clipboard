...
export const useClipboardStore = defineStore('clipboard', () => {
  ...
  const entryIndexMap = computed(
    () => new Map(entries.value.map((entry, index) => [entry.id, index])),
  );
  ...
  async function fetchEntries(reset = true) {
    ...
    try {
      ...
      const result = searchKeyword.value.trim()
        ? await invoke<SearchResult>('search_entries', {
        ...
        : await invoke<SearchResult>('get_entries', {
            limit: pageSize,
            offset,
            category,
            isFavorite: isFavoriteFilter,
  ...
  function handleEntryPrimaryAction(id: number, options?: { range?: boolean }) {
    if (isMultiSelectMode.value) {
      if (options?.range) selectRangeTo(id);
      else {
  ...
  async function favoriteSelectedEntries(favorite: boolean) {
    ...
    try {
      ...
      for (const entry of entries.value)
        if (selectedIdSet.has(entry.id)) entry.is_favorite = favorite;
