...
export const useSmartStore = defineStore('smart', () => {
  ...
  async function fetchClusterEntries(clusterId: number, limit?: number, offset?: number): Promise<ClipboardEntry[]> {
    try {
      const result = await invoke<{ entries: ClipboardEntry[]; total_count: number }>('get_cluster_entries', {
        clusterId,
        limit: limit ?? 50,
        offset: offset ?? 0,
      });
