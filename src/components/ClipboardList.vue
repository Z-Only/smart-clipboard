<template>
  <div class="flex h-full min-h-0 flex-col">
    ...
    <div
      class="flex items-center justify-between gap-3 border-b border-border px-3 py-2 text-xs text-muted-foreground"
    >
...
const visibleItems = computed(() => {
  ...
  ) as Array<ClipboardListItem & { offset: number; height: number }>;
});
const stickyGroupLabel = computed(() => {
  ...
  return current?.type === 'group'
    ? current.group.label
    : groupedEntryItems.value[0]?.type === 'group'
      ? groupedEntryItems.value[0].group.label
      : '';
});
...
function ensureActiveVisible() {
  ...
  const target = layoutItems.value.find(
    (item) => item.type === 'entry' && item.entry.id === activeEntryId.value,
  );
  if (!target) return;
...
async function handleApplyBatchTags(payload: { tagIds: number[]; mode: 'append' | 'replace' }) {
  await store.applyTagsToSelectedEntries(payload.tagIds, payload.mode);
...
watch(
  () => groupedEntryItems.value.length,
  async () => {
    await nextTick();
    updateViewportMetrics();
  },
