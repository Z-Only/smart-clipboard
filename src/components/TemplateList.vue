<template>
  ...
  >
    <div
      class="bg-background rounded-lg shadow-xl w-[600px] max-h-[80vh] flex flex-col border border-border"
    >
      <!-- Header -->
      ...
      <div
        ...
        class="flex items-center gap-1 px-4 py-2 border-b border-border overflow-x-auto"
      >
        <button
      ...
      <div class="flex-1 overflow-y-auto p-4">
        ...
        <div v-else class="space-y-2">
          ...
          >
            ...
            <div class="text-xs text-muted-foreground line-clamp-2 font-mono">
              {{ tmpl.content }}
            </div>
            <div class="flex items-center gap-2 mt-2 text-[10px] text-muted-foreground">
              ...
              <span v-if="tmpl.use_count > 0">{{
                $t('statistics.times', { n: tmpl.use_count })
              }}</span>
            </div>
          </div>
        </div>
